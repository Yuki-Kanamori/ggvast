...
# please change here --------------------------------------------
data = df_dens #VASTの結果ならdf_dens　ノミナルならData_Geostat
...
# make function -------------------------------
# !!! DO NOT CHANGE HERE !!! ------------------------------------
map_dens = function(data, data_type, region, scale_name, ncol, shape, size, map_output_dirname){
  ...
  if(data_type == "VAST"){
    ...
    if(length(unique(data$category)) != 1){
      for(i in 1:length(unique(data$category))){
        ...
        ggsave(filename = "map_dens_", unique(data2$category), ".pdf", plot = fig, units = "in", width = 8.27, height = 11.69)
      }
    }
  }
  ...
  if(data_type == "nominal"){
    ...
    if(length(unique(data$category)) != 1){
      for(i in 1:length(unique(data$category))){
        ...
        c = scale_colour_gradientn(colours = c("black", "blue", "cyan", "green", "yellow", "orange", "red", "darkred"))
        fig = local_map+theme_bw()+th+p+f+c+labs(title = paste(unique(data2$category_name)), x = "Longitude", y = "Latitude", colour = scale_name)
        ggsave(filename = paste0("map_dens_nominal_", unique(data2$category), ".pdf"), plot = fig, units = "in", width = 8.27, height = 11.69)
      }
    }
  }
}

...
#'
#' @export
map_dens = function(data, region, scale_name, ncol, shape, size, zoom_in, fig_output_dirname){
  ...
  if("category_name" %in% names(data)){
    ...
    local_map = map + geom_polygon(data = region2, aes(x = long, y = lat, group = group), colour = "black", fill = "white") + coord_map(xlim = c((min(data$lon)-1)*zoom_in*(-1), (max(data$lon)+1)*zoom_in*(-1)), ylim = c((min(data$lat)-1)*zoom_in*(-1), (max(data$lat)+1)*zoom_in*(-1)))
